...
describe("Mortgage Interest Calculations", () => {
    ...
    describe("calculateMonthlyInterestDailyMethod", () => {
        it("should calculate different interest for different month lengths", () => {
            ...
            const januaryInterest = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, januaryDate);

            // February (29 days in 2024 - leap year)
            const februaryDate = new Date(2024, 1, 1);
            const februaryInterest = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, februaryDate);

            // March (31 days)
            const marchDate = new Date(2024, 2, 1);
            const marchInterest = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, marchDate);
        ...
        it("should handle leap year correctly", () => {
            ...
            const feb2024Interest = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, feb2024);

            // February 2023 (non-leap year - 28 days)
            const feb2023 = new Date(2023, 1, 1);
            const feb2023Interest = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, feb2023);
    ...
    describe("Comparison of calculation methods", () => {
        it("should show the difference between compound and daily methods", () => {
            ...
            const dailyResult = calculateMonthlyInterestDailyMethod(balanceCents, annualRate, testDate);
    ...
    describe("Bug identification and fixes", () => {
        it("should highlight the fixed 30-day bug in the original calculation", () => {
            ...
            const janCorrect = calculateMonthlyInterestDailyMethod(balance, rate, jan2024);
            const febCorrect = calculateMonthlyInterestDailyMethod(balance, rate, feb2024);
            const aprCorrect = calculateMonthlyInterestDailyMethod(balance, rate, apr2024);
